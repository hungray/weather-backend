...
async function runCronJob() {
  ...
  try {
    ...
    console.log(`Cron finished successfully. Sent ${sentCount} notifications.`);
    return sentCount;
  } catch (error) {
...
app.get('/api/trigger-alerts', async (req, res) => {
  try {
    const sentCount = await runCronJob();
...
// إذا تم تشغيل الملف بشكل مباشر من خلال GitHub Actions
if (process.env.RUN_CRON === 'true') {
  runCronJob().then(() => {
    console.log("GitHub Actions Cron Completed");
    process.exit(0);
  }).catch((e) => {
    console.error("GitHub Actions Cron Failed", e);
    process.exit(1);
  });
} else {
